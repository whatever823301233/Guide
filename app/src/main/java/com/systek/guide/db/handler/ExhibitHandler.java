package com.systek.guide.db.handler;
...
public class ExhibitHandler {
        ...
        try {
            for (Exhibit e : exhibitList) {
                DBHandler.getInstance().getDB().execSQL(
                        "INSERT INTO "+ Exhibit.TABLE_NAME
                                +" VALUES(null,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                        new Object[]{
                                e.getId(),
                                e.getMuseumId(),
    ...
    public  static List<Exhibit> queryFavoriteExhibitListByMuseumId(String museumId) {
        ...
        Cursor c = DBHandler.getInstance().getDB().rawQuery(
                "SELECT * FROM "+ Exhibit.TABLE_NAME
                +" WHERE "+ Exhibit.MUSEUM_ID
                        + " = ? AND " + Exhibit.IS_FAVORITE + " = ?",
                new String[]{museumId,String.valueOf(1)});
        while (c.moveToNext()) {
            Exhibit e =buildExhibitByCursor(c);
            exhibitList.add(e);
        }
    ...
    public  static List<Exhibit> queryAllExhibitList() {
        ...
        Cursor c = DBHandler.getInstance().getDB().rawQuery(
                "SELECT * FROM "+ Exhibit.TABLE_NAME ,null);
        while (c.moveToNext()) {
            Exhibit e = buildExhibitByCursor(c);
            exhibitList.add(e);
        }
    ...
    public static  Exhibit queryExhibitById(String id,String museumId) {
        Cursor c = DBHandler.getInstance().getDB().rawQuery(
                "SELECT * FROM "+ Exhibit.TABLE_NAME
                        + " WHERE " + Exhibit.ID + " = ? AND "+ Exhibit.MUSEUM_ID + " = ?",
    ...
    private  static Exhibit buildExhibitByCursor(Cursor c) {
        ...
        e.setAreaRoomId(c.getString(c.getColumnIndex(Exhibit.AREA_ROOM_ID)));
